...
|-------|------|-------------|---------|
| `id` | INTEGER PRIMARY KEY AUTOINCREMENT | Auto-generated by SQLite (do NOT provide when inserting) | 1, 2, 3... |
| `timestamp` | REAL | Unix timestamp (milliseconds) | 1732233600123.0 |
| `event` | TEXT | Memory summary - automatically set to first 500 chars of content | "Testing CASCADE memory..." |
| `context` | TEXT | Full memory content (complete text) | "Testing CASCADE memory system for the first time" |
| `emotional_intensity` | REAL | Emotional weight (0.0-1.0) | 0.7 |
| `importance` | REAL | Memory importance (0.0-1.0, default: 0.5) | 0.8 |
| `frequency` | REAL | Frequency counter (default: 1, increments with recall) | 1 |
| `metadata` | TEXT | JSON-encoded additional data | '{"context":"...","importance":0.8}' |

**Important notes:**
- **ID is auto-generated** - SQLite AUTOINCREMENT handles this automatically. Never provide id when inserting.
- **Event vs Context** - The MCP automatically populates `event` with the first 500 characters of your content, and `context` with the full content.
- **Frequency field** - Tracks how often a memory is accessed (starts at 1). This is NOT related to query performance rate.
- **Query performance** - Actual query times are 5-35ms (based on production use), unrelated to the frequency field value.
...
**Note:** The `NOVA_FREQUENCY` env var (default: 21.43) is a configuration setting. The `frequency` database field (default: 1) tracks memory access count. Neither are related to query performance speed.

---

## Known Limitations

1. **No automated setup script** - Databases must be created manually (see Step 2)
2. **No built-in migration tools** - Use manual SQL or file copy for migrations
3. **Generic identity layer** - "nova" layer name is hardcoded in code; requires editing `server/index.js` to rename
4. **Auto-populated fields** - The `event` field is automatically populated with truncated content; you cannot override it directly
